...
TRY
delete_hdfs_dir("/data/custom/rb/evk/stg/ref_union_campaign_dic_hdp_snp_stg");
CATCH ex THEN
...
DROP TABLE IF EXISTS custom_rb_evk_stg.ref_union_campaign_dic_hdp_snp_stg;
CREATE EXTERNAL TABLE custom_rb_evk_stg.ref_union_campaign_dic_hdp_snp_stg(
  `sk_id` bigint, 
  `ab_test_id` bigint, 
  `product_id` string, 
  `product_name` string, 
...
STORED AS PARQUET
LOCATION '/data/custom/rb/evk/stg/ref_union_campaign_dic_hdp_snp_stg'
TBLPROPERTIES ('PARQUET.COMPRESS'='SNAPPY', 'transactional'='false');
MSCK REPAIR TABLE custom_rb_evk_stg.ref_union_campaign_dic_hdp_snp_stg;

INSERT OVERWRITE TABLE custom_rb_evk_stg.ref_union_campaign_dic_hdp_snp_stg
SELECT
	sk_id,
    ab_test_id,
    product_id,
...
FROM custom_rb_evk.ref_union_campaign_dic_hdp;

coalesce_files(
	"/data/custom/rb/evk/stg/ref_union_campaign_dic_hdp_snp_stg",
	"/data/custom/rb/evk/stg/ref_union_campaign_dic_hdp_snp_temp",
	128, "snappy", CAST($parallelDegree as INT)
);

move_table_to_schema(
    s2tTableList="ref_union_campaign_dic_hdp_snp_stg->ref_union_campaign_dic_hdp_snp",
    srcSchema="custom_rb_evk_stg",
    tgtSchema="custom_rb_evk",
    compressionType="snappy",
    workMode=$typeWf,
    truncateIncFilterList="",
    truncateArcFilterList="ref_union_campaign_dic_hdp_snp->all",
    instanceFilter="",
