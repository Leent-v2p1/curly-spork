...
TRY
delete_hdfs_dir('/data/custom/rb/sbol/stg/sbol_logon_aggr_dt_ikfl6_stg');
CATCH ex THEN
    print($ex['message']);
END

DROP TABLE IF EXISTS custom_rb_sbol_stg.sbol_logon_aggr_dt_ikfl6_stg;
CREATE EXTERNAL TABLE custom_rb_sbol_stg.sbol_logon_aggr_dt_ikfl6_stg(
...
STORED AS PARQUET
LOCATION '/data/custom/rb/sbol/stg/sbol_logon_aggr_dt_ikfl6_stg'
TBLPROPERTIES ('PARQUET.COMPRESS'='SNAPPY', 'transactional'='false');

INSERT OVERWRITE TABLE custom_rb_sbol_stg.sbol_logon_aggr_dt_ikfl6_stg PARTITION(day_part)
WITH logins AS (
  SELECT id, last_logon_card_tb AS tb_nmb, last_logon_card_osb AS osb_nmb, last_logon_card_vsp AS vsp_nmb
  FROM ${$src_schema_name}.sbol_logon_ikfl6
),
logon_base AS (
  SELECT
  ...
  FROM ${$src_schema_name}.sbol_logon_ikfl6 l
...
launcher_src AS (
  ...
  FROM (
      ...
      WHERE upper(type) = 'LOGON'
        AND (lower(cast(id_instance as string)) = '6' OR (cast(coalesce(cast(id_instance as int),0) as int)=0 AND '6'='gf'))
  ) x
...
coalesce_files('/data/custom/rb/sbol/stg/sbol_logon_aggr_dt_ikfl6_stg','/data/custom/rb/sbol/stg/sbol_logon_aggr_dt_ikfl6_temp',128,'snappy',CAST($parallelDegree as INT));

let partTruncList = '';
for part_name in (show partitions custom_rb_sbol.sbol_logon_aggr_dt_ikfl6) loop
...
move_table_to_schema(
    s2tTableList='sbol_logon_aggr_dt_ikfl6_stg->sbol_logon_aggr_dt_ikfl6',
    srcSchema='custom_rb_sbol_stg',
    tgtSchema='custom_rb_sbol',
    compressionType='snappy',
    workMode='inc',
    truncateIncFilterList='',
    truncateArcFilterList='sbol_logon_aggr_dt_ikfl6->all',
    instanceFilter='',
...
-- NOTE: existing partitions in custom_rb_sbol.sbol_logon_aggr_dt_ikfl6 for [$startDt, $endDt) should be truncated by scheduler/runtime if required.
