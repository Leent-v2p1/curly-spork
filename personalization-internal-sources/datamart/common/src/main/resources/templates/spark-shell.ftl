source ${datamart}-properties-spark.conf
...
spark-submit \
    --class ${className} \
    --properties-file ${datamart}-properties-system.conf \
    ...
    --deploy-mode client \
<#if extraJarNames??>
    --jars <#list extraJarNames as extraJarName>${extraJarName}<#sep>,</#list>\
</#if>
    --conf spark.driver.extraClassPath=<#if extraJarNames??><#list extraJarNames as extraJarName>${extraJarName}<#sep>:</#list>:</#if>\
