...
@DatamartRef(id = "custom_rb_vnv.ref_sale_channel", name = "SaleChannel", useSystemPropertyToGetId = true)
@FullReplace
public class VnvSaleChannel extends Datamart {

    private static final Logger log = LoggerFactory.getLogger(VnvSaleChannel.class);
    ...
    public Dataset<Row> buildDatamart() {
        ...
        SparkSession spark = SparkSession
                .builder()
                .appName("greenplum_stg_sale_channel")
                .enableHiveSupport()
                .getOrCreate();
        ...
        Dataset<Row> SaleChannel = spark.read().format("io.pivotal.greenplum.spark.GreenplumRelationProvider")
                .option("dbschema", "s_grnplm_vd_rozn_mpp_aaas_vd")
                .option("dbtable", "ref_sale_channel_hdp")
                .option("url", gpurl)
                .option("user", user)
        ...
        Dataset<Row> result = SaleChannel
                .select(
                        col("sale_channel_cd").cast(IntegerType).as("sale_channel_cd"),
                        col("sale_channel_id").cast(IntegerType).as("sale_channel_id"),
                        col("sale_channel_name").cast(StringType).as("sale_channel_name"),
                        col("sale_channel_source").cast(StringType).as("sale_channel_source")
                );
    ...
    public static void main(String[] args) {
        runner().run(VnvSaleChannel.class);
    }
}
