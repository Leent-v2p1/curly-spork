...
@DatamartRef(id = "custom_rozn_evo.da_evo_universe_tables_hdp", name = "Ref da_evo_universe_tables_hdp", useSystemPropertyToGetId = true)
@FullReplace
public class EvoUniverseTables extends Datamart {

    private static final Logger log = LoggerFactory.getLogger(EvoUniverseTables.class);
    ...
    public Dataset<Row> buildDatamart() {
        ...
        SparkSession spark = SparkSession
                .builder()
                .appName("greenplum_stg_evo_universe_tables_hdp")
                .enableHiveSupport()
                .getOrCreate();
        ...
        Dataset<Row> sourceTable = spark.read().format("io.pivotal.greenplum.spark.GreenplumRelationProvider")
                ...
                .option("dbtable", "da_evo_universe_tables_hdp")
                .option("url", gpurl)
                .option("user", user)
        ...
        Dataset<Row> result = sourceTable
                .select(
                        ...
                        col("join_cond").cast(StringType).as("join_cond"),
                        col("partition_values").cast(StringType).as("partition_values"),
                        col("changed_ts").cast(TimestampType).as("changed_ts"),
                        col("source_table").cast(StringType).as("source_table"),
                        col("table_alias").cast(StringType).as("table_alias"),
                        col("source_partition_col").cast(StringType).as("source_partition_col"),
    ...
    public static void main(String[] args) {
        runner().run(EvoUniverseTables.class);
    }
}