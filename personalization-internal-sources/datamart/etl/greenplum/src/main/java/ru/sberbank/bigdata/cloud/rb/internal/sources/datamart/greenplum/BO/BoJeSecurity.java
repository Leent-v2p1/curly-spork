...
@DatamartRef(id = "custom_rb_bo.v_is0121_bo_je_security_h", name = "Bo Je security", useSystemPropertyToGetId = true)
@PartialReplace(partitioning = "report_dt", saveRemover = UpdatedPartitionRemover.class)
public class BoJeSecurity extends Datamart {

    private static final Logger log = LoggerFactory.getLogger(BoJeSecurity.class);
    ...
    public Dataset<Row> buildDatamart() {
        ...
        SparkSession spark = SparkSession
                .builder()
                .appName("greenplum_stg_bo_je_security")
                .enableHiveSupport()
                .getOrCreate();
        ...
        Dataset<Row> boJeSecurity = spark.read().format("io.pivotal.greenplum.spark.GreenplumRelationProvider")
                .option("dbschema", "s_grnplm_vd_rozn_mpp_daas_bf_vd")
                .option("dbtable", "v_is0121_bo_je_security_h")
                .option("url", gpurl)
                .option("user", user)
        ...
        Dataset<Row> result = boJeSecurity
                .where(filter)
                .select(
                        col("agrmnt_id"),
                        col("balance_cost"),
