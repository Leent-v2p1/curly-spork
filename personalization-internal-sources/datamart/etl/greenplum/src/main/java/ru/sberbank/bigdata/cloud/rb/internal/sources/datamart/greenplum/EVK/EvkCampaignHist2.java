...
@DatamartRef(id = "custom_rb_evk.dm_union_campaign_history_hdp_2", name = "Справочник кампаний для ЕВК", useSystemPropertyToGetId = true)
@PartialReplace(partitioning = "start_dt", saveRemover = UpdatedPartitionRemover.class)
public class EvkCampaignHist2 extends Datamart {

    private static final Logger log = LoggerFactory.getLogger(EvkCampaignHist2.class);
    ...
    public Dataset<Row> buildDatamart() {
        ...
        SparkSession spark = SparkSession
                .builder()
                .appName("greenplum_stg_evk_campaign_hist_2")
                .enableHiveSupport()
                .getOrCreate();
        ...
        Dataset<Row> refUnionCampaignHist2 = spark.read().format("io.pivotal.greenplum.spark.GreenplumRelationProvider")
                .option("dbschema", "s_grnplm_vd_rozn_mpp_aaas_core")
                .option("dbtable", "dm_union_campaign_history_hdp_2")
                .option("url", gpurl)
                .option("user", user)
        ...
        Dataset<Row> result = refUnionCampaignHist2
                .select(
                        col("sk_id"),
                        col("epk_id"),
    ...
    public static void main(String[] args) {
        runner().run(EvkCampaignHist2.class);
    }
}