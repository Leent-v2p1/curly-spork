...
@DatamartRef(id = "custom_rb_bo.v_ref_is0121_dic_transtype", name = "Dic Trans Type", useSystemPropertyToGetId = true)
@FullReplace
public class DicTransType extends Datamart {

    private static final Logger log = LoggerFactory.getLogger(DicTransType.class);
    ...
    public Dataset<Row> buildDatamart() {
        ...
        SparkSession spark = SparkSession
                .builder()
                .appName("greenplum_stg_dic_transtype")
                .enableHiveSupport()
                .getOrCreate();
        ...
        Dataset<Row> dicTransType = spark.read().format("io.pivotal.greenplum.spark.GreenplumRelationProvider")
                .option("dbschema", "s_grnplm_vd_rozn_mpp_daas_bf_vd")
                .option("dbtable", "v_ref_is0121_dic_transtype")
                .option("url", gpurl)
                .option("user", user)
        ...
        Dataset<Row> result = dicTransType
                .select(
                        col("nid"),
                        col("nparentid"),
                        col("sname"),
                        col("details_tt_id").cast(LongType)
                );
