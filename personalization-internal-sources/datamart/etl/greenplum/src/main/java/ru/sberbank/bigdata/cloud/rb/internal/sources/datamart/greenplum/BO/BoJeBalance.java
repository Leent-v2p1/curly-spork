...
@DatamartRef(id = "custom_rb_bo.v_is0121_bo_je_balance_h", name = "Bo Je Balance", useSystemPropertyToGetId = true)
@PartialReplace(partitioning = "report_dt", saveRemover = UpdatedPartitionRemover.class)
public class BoJeBalance extends Datamart {

    private static final Logger log = LoggerFactory.getLogger(BoJeBalance.class);
    ...
    public Dataset<Row> buildDatamart() {
        ...
        SparkSession spark = SparkSession
                .builder()
                .appName("greenplum_stg_bo_je_balance")
                .enableHiveSupport()
                .getOrCreate();
        ...
        Dataset<Row> boJeBalance = spark.read().format("io.pivotal.greenplum.spark.GreenplumRelationProvider")
                .option("dbschema", "s_grnplm_vd_rozn_mpp_daas_bf_vd")
                .option("dbtable", "v_is0121_bo_je_balance_h")
                .option("url", gpurl)
                .option("user", user)
        ...
        Dataset<Row> result = boJeBalance
                .where(filter)
                .select(
                        col("agrmnt_id"),
                        col("blocked_"),
