...
@DatamartRef(id = "custom_rozn_evo.v_da_evo_kpi_scg", name = "Ref v_da_evo_kpi_scg_hdp", useSystemPropertyToGetId = true)
@PartialReplace(partitioning = "report_dt", saveRemover = UpdatedPartitionRemover.class)

public class EvoKpiScg extends Datamart {

    private static final Logger log = LoggerFactory.getLogger(EvoKpiScg.class);
    ...
    public Dataset<Row> buildDatamart() {
        ...
        SparkSession spark = SparkSession
                .builder()
                .appName("greenplum_stg_v_da_evo_kpi_scg")
                .enableHiveSupport()
                .getOrCreate();
        ...
        Dataset<Row> sourceTable = spark.read().format("io.pivotal.greenplum.spark.GreenplumRelationProvider")
                ...
                .option("dbtable", "v_da_evo_kpi_scg_hdp")
                .option("url", gpurl)
                .option("user", user)
        ...
        Dataset<Row> result = sourceTable
                .select(
                        ...
                        col("fact_cnt").cast(DoubleType).as("fact_cnt"),  
                        col("plan_cnt").cast(DoubleType).as("plan_cnt"),  
                        col("forecast").cast(DoubleType).as("forecast"),  
