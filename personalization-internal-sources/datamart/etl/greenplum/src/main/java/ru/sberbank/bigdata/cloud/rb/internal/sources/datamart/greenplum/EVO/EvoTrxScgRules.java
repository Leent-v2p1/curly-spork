...
@DatamartRef(id = "custom_rozn_evo.da_evo_trx_scg_rules_hdp", name = "Ref da_evo_trx_scg_rules_hdp", useSystemPropertyToGetId = true)
@FullReplace
public class EvoTrxScgRules extends Datamart {

    private static final Logger log = LoggerFactory.getLogger(EvoTrxScgRules.class);
    ...
    public Dataset<Row> buildDatamart() {
        ...
        SparkSession spark = SparkSession
                .builder()
                .appName("greenplum_stg_evo_trx_scg_rules_hdp")
                .enableHiveSupport()
                .getOrCreate();
        ...
        Dataset<Row> sourceTable = spark.read().format("io.pivotal.greenplum.spark.GreenplumRelationProvider")
                ...
                .option("dbtable", "da_evo_trx_scg_rules_hdp")
                .option("url", gpurl)
                .option("user", user)
        ...
        Dataset<Row> result = sourceTable
                .select(
                        ...
                        col("portf_scope").cast(StringType).as("portf_scope"),
                        col("portf_value").cast(StringType).as("portf_value"),
                        col("stat_cg_mark").cast(StringType).as("stat_cg_mark"),
                        col("portf_marked_value").cast(StringType).as("portf_marked_value"),
                        col("npv").cast(StringType).as("npv"),
                        col("changed_ts").cast(TimestampType).as("changed_ts"),
                        col("camp_code_mask").cast(StringType).as("camp_code_mask"),
    ...
    public static void main(String[] args) {
        runner().run(EvoTrxScgRules.class);
    }
}