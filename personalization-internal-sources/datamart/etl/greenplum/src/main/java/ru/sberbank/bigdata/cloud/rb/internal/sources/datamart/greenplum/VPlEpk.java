...
@DatamartRef(id = "custom_rb_greenplum.ft_client_aggr_mnth_pnl", name = "V Pl Epk", useSystemPropertyToGetId = true)
@PartialReplace(partitioning = "report_dt", saveRemover = UpdatedPartitionRemover.class)
public class VPlEpk extends Datamart {

    private static final Logger log = LoggerFactory.getLogger(VPlEpk.class);
    ...
    public Dataset<Row> buildDatamart() {
        ...
        SparkSession spark = SparkSession
                .builder()
                .appName("greenplum_stg_v_pl_epk")
                .enableHiveSupport()
                .getOrCreate();
        ...
        Dataset<Row> vPlEpk = spark.read().format("io.pivotal.greenplum.spark.GreenplumRelationProvider")
                .option("dbschema", "s_grnplm_vd_rozn_mpp_daas_vd")
                .option("dbtable", "v_pl_epk")
                .option("url", gpurl)
                .option("user", user)
